...
type ProductBackgroundProps = {
  color?: string;
};

const ProductBackgroundStyle = styled.div`
  position: absolute;
  top: 0;
  left: 0;
  z-index: -1;
...
const ProductBackground: FC<ProductBackgroundProps> = ({
  color = '#1698D9',
}) => {
  return (
    <ProductBackgroundStyle>
      <svg viewBox='0 0 320 480' fill='none' xmlns='http://www.w3.org/2000/svg'>
        <mask id='path-1-inside-1_78_108' fill='white'>
          <path
            fillRule='evenodd'
            clipRule='evenodd'
        ...
        <path
          ...
          fill={color}
          mask='url(#path-1-inside-1_78_108)'
        />
      </svg>
    </ProductBackgroundStyle>
  );
